...
- средняя длительность UI-теста в каждом run считается как суммарное время
  UI-тестов / количество UI-тестов; для API используется такая же формула,
  после чего значения усредняются по runs; **Total UI Test Time** — средняя
  сумма длительностей всех UI-тестов в run (объем тестовой работы, а не
  wall-clock), API run duration считается аналогично;
...
Дополнительный блок **Cross-Browser UI** сравнивает Chromium, Firefox и WebKit,
не дублируя весь dashboard. Для каждого браузера показаны pass rate, количество
падений, flaky rate, средняя и P95 длительность теста, а также P90 суммарного
времени browser-shard. Два дополнительных графика показывают pass rate и
среднюю длительность по runs. Browser Coverage проверяет, что одинаковые
логические UI-сценарии выполнялись во всех трех браузерах, а
Browser-Specific Failures агрегирует падения по тесту и браузеру. Индивидуальные
лимиты скорости задаются в `browser_targets` того же JSON-файла; общие лимиты
на надежность остаются едиными.
...
нет Allure report, он явно показывается как `No report` в таблице запусков, но
не подмешивается в тестовую статистику.
