...
- `tests/ui/identity_access/test_user_creation.py::test_admin_can_create_user`;
- `tests/ui/identity_access/test_user_creation.py::test_admin_can_delete_user`;
- `tests/ui/identity_access/test_access_tokens.py::test_admin_can_create_access_token_for_user`;
...
В CI 6 API-сценариев выполняются по одному разу, а каждый из 5 UI-сценариев —
отдельно в Chromium, Firefox и WebKit. Поэтому 11 логических smoke-сценариев
отображаются как 21 test item. Три браузера работают в независимых matrix jobs,
а финальная job объединяет результаты с API в один Allure report. В Suites они
видны в ветках `UI · Chromium`, `UI · Firefox` и `UI · WebKit`; параметр
`browser_name` сохраняет отдельную историю каждого варианта.

UI login smoke проверяет настоящую форму входа и шапку TeamCity. Остальные UI
smoke-сценарии получают browser session через REST API: создают Project и User,
а также access token для подготовленного пользователя. Созданные сущности
проверяются в UI и удаляются через API-fixtures. UI-тесты используют маркеры
